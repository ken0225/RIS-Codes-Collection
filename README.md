...
An Electronic Reliability Perspective</a>. Compared with previous works, this paper introduced a **NEW** non-residual impairment factor in the RIS-aided system, named the stochastic hardware aging (HA) effect, which describes runtime-related hardware degradations and failures. To the best knowledge of the authors, this is the first work on operation (running) times of the RIS, which reveals a **NEW** dimension (and metric) for analyzing and evaluating the RIS-aided system performance.
